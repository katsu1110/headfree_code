...
%                disp([files(f).name ' ...getExParams error'])
        ...
        try
            ...
            data.session(l).survival.raw_median = nanmedian(x);
            data.session(l).survival.auc = trapz(data.session(l).survival.edges, ...
                data.session(l).survival.survival);
            T = 0:0.001:data.session(l).params.fixdur;
            y = exp(-(T/data.session(l).survival.fitparams(1)).^data.session(l).survival.fitparams(2));
            area = 0;
            half_t = 1;
            while area < data.session(l).survival.auc/2
                area = area + y(half_t);
                half_t = half_t + 1;
            end
            data.session(l).survival.half_t = T(half_t);
        catch
