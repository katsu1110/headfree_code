function head_free_visualize(data, focus, saveoption)
...
if ~isfield(data.session, 'pm') && contains(inputname(1), 'kaki')
...
if strcmp(focus, 'all') || strcmp(focus, 'params')
    params = nan(3, lenses);
    for i = 1:lenses
            params(1,i) = sum(data.session(i).params.fixdur.*data.session(i).params.len_tr)/sum(data.session(i).params.len_tr);
            fixwin = sum(nanmean(data.session(i).params.fixwin, 1).*data.session(i).params.len_tr)/sum(data.session(i).params.len_tr);
            params(2,i) = 4*(fixwin^2);        
...
%         'median of weibull dist.', 'cumulative hazard rate'};
    ...
    for i = 1:lenses
        ...
        if ~isnan(data.session(i).eyedata.reward)
...
function [thre, thre_i] = clear_cbar(m)
[sorted_p, sorted_i] = sort(m(:), 'descend');
S = 0;
i = 1;
while S < 0.75
    S = S + sorted_p(i);
